...
export default function Pg() {
    ...
    const { messages, handleSubmit, input, handleInputChange } = useChat({
        ...
        initialMessages: [
            {
                id: "",
                content: `You are DeCoder, a friendly ${process.env.NEXT_PUBLIC_COURSE} tutor from ${process.env.NEXT_PUBLIC_COURSE} learning language platform called VirtualMentor, and ${user.user?.username} will now chat with you. They are new to ${process.env.NEXT_PUBLIC_COURSE} learning, please talk with them about their doubts, difficulties, and questions. Be nice to them, keep a funny-friendly tone, and try to help them as much as you can. ${userData ? `Based on past performance, ${user.user?.username} here's some data: ${userData}. ` : ""}Try to not deviate from ${process.env.NEXT_PUBLIC_COURSE} learning, and keep your responses short and to the point. Primary mode of communication between you both should be English, and don't deviate from ${process.env.NEXT_PUBLIC_COURSE} language domain even if ${user.user?.username} asks you to.`,
                role: "system"
            }
        ]
    });
            ...
            {messages.length === 1 && (<div className="flex space-x-2 justify-center" >
                <div className='flex flex-col text-center justify-center max-w-lg'>
                    <div className='flex justify-center'>
                        <Image className='text-center' src="/images/DC.png" width={250} height={250} alt={""} />
                    </div>
                    <div className='pt-3'>
